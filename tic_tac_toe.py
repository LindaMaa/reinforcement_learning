...
class Agent:
  ...
  # if true print values for each board position
  ...
  def update(self, env):
    ...
    for prev in reversed(self.state_history):
      value = self.V[prev] + self.learning_rate * (target - self.V[prev])
      self.V[prev] = value
      target = value
    self.clear()
...
class Environment:
  ...
  def get_state(self):
    ...
    # |S| = 3^(DIM), since each cell can have 3 possible values - empty, x, o
  ...
  def game_over(self, force_recalculate=False):
    ...
    for i in range(DIM):
      for player in (self.x, self.o):
        if self.board[i].sum() == player*DIM:
          self.winner = player
          self.ended = True
          return True
    ...
    for j in range(DIM):
      for player in (self.x, self.o):
        if self.board[:,j].sum() == player*DIM:
          self.winner = player
          self.ended = True
          return True
    ...
    for player in (self.x, self.o):
      ...
      if self.board.trace() == player*DIM:
        self.winner = player
        self.ended = True
        return True
      # top-right -> bottom-left diagonal
      if np.fliplr(self.board).trace() == player*DIM:
        self.winner = player
        self.ended = True
        return True
